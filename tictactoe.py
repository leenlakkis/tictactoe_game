...
symbols = {1: 'X', 2: 'O'}  # Default symbols for each player


def print_board(board):
    """
    This function prints the board. Each row of the board is printed on a new line.
...
def check_tie(board):
    """
    Checks whether the board is completely filled without any winner.
...
def take_turn(player, board, symbol):
    """
    ...
    :param symbol: The symbol chosen by the player ('X' or 'O').
    :return: None.
    """
...
def main():
    ...
    symbols[1] = player1_symbol
    symbols[2] = 'O' if player1_symbol == 'X' else 'X'
